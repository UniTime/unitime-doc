...
```
 ...
 Besides of the parameters, it can access a hibernate session (parameter hibSession of org.hibernate.Session class), current academic session (parameter session, of [Session](https://code.google.com/p/unitime/source/browse/trunk/JavaSource/org/unitime/timetable/model/Session.java) class) and the object running the script (parameter log, of [ScriptExecution](https://code.google.com/p/unitime/source/browse/trunk/JavaSource/org/unitime/timetable/server/script/ScriptExecution.java) class). The hibernate session is opened with a transaction started. This transaction is committed when the script finishes fine, it is rollbacked otherwise. The log can be used to print debug, info, warn, and error message; to control script status and progress; and to create an output file.


 The parameters can be of various types. Basic types are string, boolean, int, long, double, float, short, and byte. There can be a drop down selection of one or multiple departments (type department or departments of class [Department](https://code.google.com/p/unitime/source/browse/trunk/JavaSource/org/unitime/timetable/model/Department.java) or List<Department>), one or multiple subject areas (type subject or subjects of class [SubjectArea](https://code.google.com/p/unitime/source/browse/trunk/JavaSource/org/unitime/timetable/model/SubjectArea.java) or List<SubjectArea>), one or multiple buildings (type building or buildings of class [Building](https://code.google.com/p/unitime/source/browse/trunk/JavaSource/org/unitime/timetable/model/Building.java) or List<Building>), one or more locations (type location or locations of class [Location](https://code.google.com/p/unitime/source/browse/trunk/JavaSource/org/unitime/timetable/model/Location.java) or List<Location>), or one or more rooms (type room or rooms of class [Room](https://code.google.com/p/unitime/source/browse/trunk/JavaSource/org/unitime/timetable/model/Room.java) or List<Room>). It can also be an input file (type file). A selection of one of given strings (type enum) or a value from a reference table (type reference(RefTableEntry), where RefTableEntry is a model class inherited from [RefTableEntry](https://code.google.com/p/unitime/source/browse/trunk/JavaSource/org/unitime/timetable/model/RefTableEntry.java), the result is a string with the value of the reference column of the selected item) are also accepted.
